...
//   }
  ...
  let text = `请从下面简历文本内容中，按照模版 ${template} 简历JSON结构,返回结构化json。注意：1.如果模版中字段不够，请自行创建新字段。 2.如果模版中字段没有匹配到合适内容，也要返回这个字段,字段为空字符串。3.不需要创建英文字段(例如name:"张三",name_en:"zhang san")。4.json最后添加一个字段以及值是"Warning":"仅限于招聘使用"`
